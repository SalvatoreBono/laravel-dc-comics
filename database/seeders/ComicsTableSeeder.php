...
class ComicsTableSeeder extends Seeder
{
    ...
    public function run(): void
    {
        foreach ($this->comics as $comic) {
            ...
            $newSingleComic->title = $comic["title"];
            $newSingleComic->description = $comic["description"];
            $newSingleComic->thumb = $comic["thumb"];
            $newSingleComic->price = str_replace("$", "", $comic["price"]);
            $newSingleComic->series = $comic["series"];
            $newSingleComic->sale_date = $comic["sale_date"];
            $newSingleComic->type = $comic["type"];
            $newSingleComic->artists = json_encode($comic["artists"]);
            $newSingleComic->writers = json_encode($comic["writers"]);
